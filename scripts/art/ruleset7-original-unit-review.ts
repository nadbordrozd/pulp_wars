...
if (process.argv.includes("--revision3")) {
  const outputIndex = process.argv.indexOf("--output");
  const output = outputIndex < 0 ? undefined : process.argv[outputIndex + 1];
  if (output === undefined)
    throw new Error("--revision3 requires --output <directory>");
  await createRevision3HorseArcherReview(path.resolve(root, output));
  process.exit(0);
}
...
async function createRevision3HorseArcherReview(
  outputRoot: string,
): Promise<void> {
  const id = "unit-original-horse-archer";
  const portraitId = "portrait-original-horse-archer";
  const recipe = required(
    source.recipes.find((candidate) => candidate.id === id),
    `${id} recipe`,
  );
  const record = required(generated.records[id], `${id} generated record`);
  if (!new Set(["CANDIDATE", "ACCEPTED"]).has(record.status))
    throw new Error(`${id}: expected CANDIDATE or ACCEPTED`);
  const file =
    record.status === "ACCEPTED"
      ? recipe.output
  ...
  const expectedHash =
    record.status === "ACCEPTED" ? record.outputSha256 : record.candidateSha256;
  if (hash(bytes) !== expectedHash) throw new Error(`${id}: hash drifted`);
  ...
  const scale = 0.27;
  const offsetY = 18;
  const nativeCanvas = await sharp(bytes)
  ...
  const enlarged = await sharp(bytes)
    .resize(info.width * 2, info.height * 2, { fit: "fill" })
    .png()
    .toBuffer();
  await mkdir(outputRoot, { recursive: true });

  await writeRevision3Canvas(
    outputRoot,
    1260,
    850,
    [
      { input: checker(384, 384), left: 24, top: 90 },
      { input: bytes, left: 24, top: 90 },
      {
        input: revision3Label("SOURCE 384×384", hash(bytes), 384),
        left: 24,
        top: 18,
      },
      { input: checker(768, 768), left: 438, top: 70 },
      { input: enlarged, left: 438, top: 70 },
      {
        input: revision3Label("ENLARGED 2×", "inspect outline + alpha", 768),
        left: 438,
        top: 0,
      },
      { input: checker(128, 128), left: 125, top: 580 },
      { input: nativeCanvas, left: 138, top: 593 },
      {
        input: revision3Label("NATIVE 0.27", "104×104 canvas", 320),
        left: 30,
        top: 720,
      },
    ],
    "horse-archer-source-native-enlarged.png",
  );

  const comparison = [
    {
      label: "Lancer 0.25",
      file: "public/assets/pixellab/units/original-lancer.png",
      width: 64,
      height: 74,
      anchorX: 32,
      anchorY: 55.5,
    ...
    {
      label: "Raider 0.25",
      file: "public/assets/pixellab/units/rider.png",
      width: 64,
      height: 74,
      anchorX: 32,
      anchorY: 55.5,
    ...
    {
      label: "Catapult 0.24",
      file: "public/assets/pixellab/units/original-catapult.png",
      width: 92,
      height: 92,
      anchorX: 46.08,
      anchorY: 69.12,
  ...
  const comparisonOverlays: OverlayOptions[] = [];
  for (const [index, item] of comparison.entries()) {
    const centerX = 110 + index * 190;
    const groundY = 250;
    const sprite = await sharp(path.join(root, item.file))
      .resize(item.width, item.height, { fit: "fill" })
      .png()
      .toBuffer();
    comparisonOverlays.push(
      {
        input: outlineSvg(128, "#73988d", 2),
        left: centerX - 64,
        top: groundY - 64,
      },
      {
        input: sprite,
        left: Math.round(centerX - item.anchorX),
        top: Math.round(groundY - item.anchorY + item.offset),
      },
      {
        input: revision3Label(item.label, "shared +18 CSS baseline", 170),
        left: centerX - 85,
        top: 340,
      },
    );
  ...
  await writeRevision3Canvas(
    ...
    430,
    comparisonOverlays,
    "horse-archer-class-comparison.png",
  );
  ...
  for (const [index, terrainId] of terrainIds.entries()) {
    const terrainRecipe = required(
      source.recipes.find(({ id: candidateId }) => candidateId === terrainId),
      `${terrainId} recipe`,
    );
    const centerX = 150 + index * 260;
    const centerY = 230;
    const terrainBytes = await readFile(path.join(root, terrainRecipe.output));
    const terrainMeta = await sharp(terrainBytes).metadata();
    const terrainWidth = Math.round((terrainMeta.width ?? 256) * 0.5);
    const terrainHeight = Math.round((terrainMeta.height ?? 256) * 0.5);
    const terrainAnchorY = (terrainRecipe.anchor?.y ?? 128) * 0.5;
    contextOverlays.push(
      {
        input: await sharp(terrainBytes)
          .resize(terrainWidth, terrainHeight, { fit: "fill" })
          .png()
          .toBuffer(),
        left: centerX - 64,
        top: Math.round(centerY - terrainAnchorY),
      },
      {
        input: nativeCanvas,
        left: centerX - Math.round(192 * scale),
        top: Math.round(centerY - 288 * scale + offsetY),
      },
      {
        input: revision3Label(terrainId, "native square context", 240),
        left: centerX - 120,
        top: 345,
      },
    );
  ...
  await writeRevision3Canvas(
    ...
    430,
    contextOverlays,
    "horse-archer-terrain-contexts.png",
  );

  const portraitRecord = generated.records[portraitId];
  const portraitRecipe = source.recipes.find(
    ({ id: candidateId }) => candidateId === portraitId,
  );
  if (
    portraitRecipe !== undefined &&
    portraitRecord !== undefined &&
    new Set(["CANDIDATE", "ACCEPTED"]).has(portraitRecord.status)
  ) {
    const portraitFile =
      portraitRecord.status === "ACCEPTED"
        ? portraitRecipe.output
        : required(portraitRecord.candidate, `${portraitId} candidate`);
    const portraitBytes = await readFile(path.join(root, portraitFile));
    const uiPreview = await sharp(portraitBytes)
      .resize(112, 130, { fit: "contain", background: "#00000000" })
      .png()
      .toBuffer();
    const nominal = await sharp(portraitBytes)
    ...
    await writeRevision3Canvas(
      ...
      390,
      [
        { input: checker(256, 256), left: 30, top: 80 },
        { input: portraitBytes, left: 30, top: 80 },
        {
          input: revision3Label("PORTRAIT SOURCE", hash(portraitBytes), 256),
          left: 30,
          top: 8,
        },
        { input: checker(112, 130), left: 370, top: 120 },
        { input: uiPreview, left: 370, top: 120 },
        {
          input: revision3Label("112×130 UI", "object-fit contain", 220),
          left: 316,
          top: 48,
        },
        { input: checker(64, 64), left: 610, top: 150 },
        { input: nominal, left: 610, top: 150 },
        {
          input: revision3Label("64×64", "nominal portrait", 150),
          left: 567,
          top: 245,
        },
      ],
      "horse-archer-portrait.png",
    );
  }
  ...
  const artifactNames = [
    ...
    "horse-archer-class-comparison.png",
    "horse-archer-terrain-contexts.png",
    ...(portraitRecord === undefined ? [] : ["horse-archer-portrait.png"]),
  ];
  const artifacts = await Promise.all(
    artifactNames.map(async (filename) => {
      const artifact = await readFile(path.join(outputRoot, filename));
  ...
  await writeFile(
    ...
    `${JSON.stringify(
      {
        schemaVersion: 1,
        mode: "RULESET7_REVISION3_HORSE_ARCHER",
        status: record.status,
        id,
        sourcePath: file,
        sourceSha256: hash(bytes),
        rejectedAttempts: record.rejectedAttempts ?? [],
        portraitStatus: portraitRecord?.status ?? "NOT_DERIVED",
        geometry: {
          source: { width: info.width, height: info.height },
          anchor: { x: 192, y: 288 },
          displayScale: scale,
          cosmeticOffsetCss: offsetY,
          alphaBounds: bounds,
          visibleCss: {
            width: Number(((bounds.right - bounds.left) * scale).toFixed(3)),
            height: Number(((bounds.bottom - bounds.top) * scale).toFixed(3)),
          },
          roughMeaningfulBodyGuidanceCss: {
            width: "72..82",
            height: "60..72",
            interpretation:
              "body guidance, not a hard full-alpha cap; accepted horse, rider and bow silhouette measures 73.98 by 73.44 CSS",
          },
          sourceContactBottom: bounds.bottom,
          owningSquareOverflowCss: {
            left: Math.max(
              0,
              Number(((192 - bounds.left) * scale - 64).toFixed(3)),
            ),
            right: Math.max(
              0,
              Number(((bounds.right - 192) * scale - 64).toFixed(3)),
            ),
            bottom: Math.max(
              0,
              Number(((bounds.bottom - 288) * scale + offsetY - 64).toFixed(3)),
            ),
          },
        },
        comparisonClasses: comparison.map(
          ({ label: comparisonLabel }) => comparisonLabel,
        ),
        contexts: terrainIds,
        artifacts,
      },
...
async function writeRevision3Canvas(
  outputRoot: string,
  width: number,
  height: number,
...
function checker(width: number, height: number): Buffer {
  ...
  for (let y = 0; y < height; y += 16)
    for (let x = 0; x < width; x += 16)
      cells.push(
        `<rect x="${x}" y="${y}" width="16" height="16" fill="${(x / 16 + y / 16) % 2 === 0 ? "#d9e0dc" : "#aebbb5"}"/>`,
      );
  return Buffer.from(
...
function revision3Label(
  heading: string,
  detail: string,
  width: number,
): Buffer {
  return Buffer.from(
    `<svg xmlns="http://www.w3.org/2000/svg" width="${width}" height="64"><text x="${width / 2}" y="25" text-anchor="middle" font-family="sans-serif" font-size="18" font-weight="700" fill="#fff7e7">${escapeXml(heading)}</text><text x="${width / 2}" y="49" text-anchor="middle" font-family="monospace" font-size="12" fill="#b8d1ca">${escapeXml(detail.slice(0, 44))}</text></svg>`,
  );
}
