...
try {
  ...
  await evaluate(
    connection,
    `(() => {
      localStorage.removeItem('pulpWars.save.v7r8.current');
  ...
  const preview = await evaluate<PreviewEvidenceV7>(
    ...
    `(() => {
      ...
      const save = JSON.parse(localStorage.getItem('pulpWars.save.v7r8.current') ?? 'null');
